...
@Typed class Rename extends RedisCommand {
    String   oldKey, newKey

    protected ChannelFuture write(Channel channel) {
        channel.write("RENAME $oldKey $newKey\r\n".channelBuffer)
    }

    protected void buildCommand (RedisCommandBuilder command) {
        command.addString(oldKey)
        command.addString(newKey)
    }
    ...
    String toString () { "RENAME($oldKey,$newKey)"}
}
